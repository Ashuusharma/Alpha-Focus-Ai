...
export default function AssessmentPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F4EFE6] flex items-center justify-center px-6">
        <div className="rounded-2xl bg-white border border-[#E2DDD3] px-6 py-5 text-sm text-[#6B665D]">Loading clinical flow validation...</div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-[#F4EFE6] flex items-center justify-center px-6">
        <div className="max-w-xl w-full rounded-3xl bg-white border border-[#E2DDD3] p-8 text-center space-y-4">
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-full bg-[#1F3D2B] px-6 py-3 text-sm font-semibold text-white"
          >
            Go to Analyzer
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#F4EFE6]">
      <div className="sticky top-0 z-30 bg-[#F4EFE6]/95 backdrop-blur-md border-b border-[#E2DDD3]">
        <div className="max-w-3xl mx-auto px-6 py-4 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-lg font-bold text-[#1F3D2B]">Clinical Assessment - {getCategoryLabel(activeCategory)}</h1>
              <p className="text-xs text-[#6B665D]">Category-locked protocol scoring with weighted domain inputs.</p>
            </div>
            <p className="text-xs font-semibold text-[#2F6F57]">{answeredCount}/{categoryQuestions.length} answered</p>
          </div>
          <div className="rounded-xl border border-[#E2DDD3] bg-white px-3 py-2 text-xs text-[#4A453E]">
          ...
          {flowDiagnosticSource && (
            <p className="text-[11px] text-[#6B665D]">
              Diagnostic mode: {flowDiagnosticSource === "db_scan" ? "DB scan validated" : "Session fallback"}
            </p>
          )}
          <div className="w-full h-1.5 bg-[#E2DDD3] rounded-full overflow-hidden">
            <div className="h-full bg-[#2F6F57]" style={{ width: `${progressPercent}%` }} />
          </div>
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          {activeQuestion && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -12 }}
              className="rounded-3xl bg-white border border-[#E2DDD3] p-6 md:p-8 space-y-6"
            >
              <div className="flex items-center justify-between">
                <button
                  onClick={handleBack}
                  className="inline-flex items-center gap-2 text-xs font-semibold text-[#6B665D] hover:text-[#1F3D2B]"
                >
                    ...
                    <button
                      ...
                      className={`w-full rounded-xl border px-4 py-3 text-left transition ${
                        selected
                          ...
                          : "border-[#E2DDD3] bg-white text-[#1F3D2B] hover:border-[#2F6F57]/40"
                      }`}
                    >
                      <div className="flex items-center justify-between gap-3">
                        <span className="text-sm font-medium">{option.label}</span>
                        <span className={`text-xs font-semibold ${selected ? "text-white/90" : "text-[#6B665D]"}`}>Score {option.score}</span>
                      </div>
                    </button>
                  );
                })}
              ...
              <div className="pt-2 flex items-center justify-end">
                {!isLastQuestion ? (
                  <button
                    ...
                    disabled={!isAnswered}
                    className="rounded-full bg-[#1F3D2B] px-6 py-2.5 text-sm font-semibold text-white disabled:opacity-50"
                  >
                    Next Question
                  </button>
                ) : (
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-full bg-[#2F6F57] px-6 py-2.5 text-sm font-semibold text-white disabled:opacity-50"
                  >
                    <CheckCircle2 className="w-4 h-4" />
                    {isSubmitting ? "Submitting..." : "Submit & Generate Clinical Report"}
                  </button>
                )}
              </div>
            </motion.div>
