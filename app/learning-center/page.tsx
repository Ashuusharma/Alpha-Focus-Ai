...
export default function LearningCenterPage() {
  ...
  return (
    <div className="af-page-shell py-12">
      <Container>
        <div className="af-page-frame mx-auto max-w-6xl">
          ...
          <section className="nv-section-white">
            ...
            <button
              onClick={() => router.back()}
              className="group inline-flex items-center gap-2 rounded-full px-4 py-2 text-[var(--ink)] transition-colors hover:bg-white hover:text-[var(--ink)]"
            >
            ...
            <div className="space-y-3">
              ...
              <h1 className="text-clinical-heading text-4xl font-extrabold tracking-tight text-[var(--ink)]">Clinical guidance, protocol education, and category-specific execution in one hub.</h1>
              <p className="max-w-2xl text-sm font-medium leading-7 text-[var(--ink-soft)]">The learning center now carries the same premium shell as the operational pages, so education sits inside the product journey instead of outside it.</p>
            </div>
            <div className="grid gap-4 md:grid-cols-3">
              <div className="af-stat-tile">
                <p className="text-[11px] font-black uppercase tracking-[0.16em] text-[var(--ink-soft)]">Knowledge paths</p>
                <p className="mt-2 text-3xl font-bold text-[var(--ink)]">3</p>
                <p className="mt-1 text-xs text-[var(--ink-soft)]">Baseline, execution, optimization</p>
              </div>
              <div className="af-stat-tile">
                <p className="text-[11px] font-black uppercase tracking-[0.16em] text-[var(--ink-soft)]">Categories covered</p>
                <p className="mt-2 text-3xl font-bold text-[var(--ink)]">{categories.length}</p>
                <p className="mt-1 text-xs text-[var(--ink-soft)]">Mapped to assessment prompts</p>
              </div>
              <div className="af-stat-tile">
                <p className="text-[11px] font-black uppercase tracking-[0.16em] text-[var(--ink-soft)]">Best use cadence</p>
                <p className="mt-2 text-base font-semibold text-[var(--ink)]">Weekly before routine changes</p>
                <p className="mt-1 text-xs text-[var(--ink-soft)]">Use with report and scan reviews</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-3">
              <button onClick={() => router.push("/assessment")} className="af-quick-action border border-[var(--accent-blue)] text-[var(--accent-blue)] bg-white">Answer Questions</button>
              <button onClick={() => router.push("/image-analyzer")} className="af-quick-action border border-[var(--accent-blue)] text-[var(--accent-blue)] bg-white">Analyze Photo</button>
          ...
          <section className="af-card-secondary p-8">
            <h2 className="text-2xl font-bold text-[var(--ink)] mb-2">Protocol Learning Paths</h2>
            <p className="text-sm text-[var(--ink-soft)] mb-6">Choose your current phase and follow the shortest path to execution quality.</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="rounded-xl border border-[var(--border-hairline)] bg-[var(--tint-neutral)] p-4">
                <p className="text-xs uppercase tracking-wider text-[var(--ink-soft)]">Path 1</p>
                <p className="font-semibold mt-1">Baseline Week</p>
                <p className="text-xs text-[var(--ink-soft)] mt-1">Focus on analyzer quality + complete first full assessment cycle.</p>
                <button onClick={() => router.push("/image-analyzer")} className="mt-3 rounded-lg bg-[var(--accent-blue)] px-3 py-2 text-xs font-semibold text-white">Start Baseline</button>
              </div>
              <div className="rounded-xl border border-[var(--border-hairline)] bg-[var(--tint-neutral)] p-4">
                <p className="text-xs uppercase tracking-wider text-[var(--ink-soft)]">Path 2</p>
                <p className="font-semibold mt-1">Execution Week</p>
                <p className="text-xs text-[var(--ink-soft)] mt-1">Use challenge loops to convert your report into daily completed actions.</p>
                <button onClick={() => router.push("/challenges")} className="mt-3 rounded-lg bg-[var(--accent-blue)] px-3 py-2 text-xs font-semibold text-white">Open Challenges</button>
              </div>
              <div className="rounded-xl border border-[var(--border-hairline)] bg-[var(--tint-neutral)] p-4">
                <p className="text-xs uppercase tracking-wider text-[var(--ink-soft)]">Path 3</p>
                <p className="font-semibold mt-1">Optimization Week</p>
                <p className="text-xs text-[var(--ink-soft)] mt-1">Refine routine and product fit based on measurable trend movement.</p>
                <button onClick={() => router.push("/dashboard")} className="mt-3 rounded-lg bg-[var(--accent-blue)] px-3 py-2 text-xs font-semibold text-white">Track Progress</button>
              </div>
            </div>
          </section>
              ...
              return (
                <div key={item.id} className="cursor-pointer" onClick={() => setSelectedGuide(isOpen ? null : item.id)}>
                   <div className={`relative border transition-all duration-300 overflow-hidden ${
                     isOpen 
                       ? "bg-white border-[var(--accent-blue)]" 
                       : "bg-white border-[var(--border-hairline)]"
                   }`}>
                    <div className="p-6">
                      <div className="flex items-center gap-4 mb-4">
                        <span className={`p-3 rounded-xl border transition-colors ${
                          isOpen ? "bg-transparent border-[var(--accent-blue)] text-[var(--accent-blue)]" : "bg-white border-[var(--border-hairline)] text-[var(--ink)]"
                        }`}>
                          <item.icon className="w-6 h-6" />
                        </span>
                        <h3 className={`font-semibold text-lg leading-tight ${isOpen ? "text-[var(--ink)]" : "text-[var(--ink)]"}`}>{item.title}</h3>
                      </div>
                      <div className={`text-sm text-[var(--ink-soft)] leading-relaxed transition-all duration-300 ${isOpen ? "opacity-100 max-h-40" : "opacity-0 max-h-0 overflow-hidden"}`}>
                        {item.text}
                      </div>
                      <div className={`absolute top-6 right-6 transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`}>
                         <ChevronDown className="w-5 h-5 text-[var(--accent-blue)]" />
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="af-card-secondary p-8">
            <h2 className="text-2xl font-bold text-[var(--ink)] mb-2">Category Knowledge Library</h2>
            <p className="text-[var(--ink-soft)] mb-8">Expand each domain to read focused guidance and key question themes.</p>
              ...
              {categories.map((category) => {
                ...
                return (
                  <div key={category.id} className={`rounded-2xl border transition-all duration-300 overflow-hidden ${
                    isOpen
                        ? "bg-white border-[var(--accent-blue)] shadow-md"
                        : "bg-white border-[var(--border-hairline)] hover:bg-white"
                  }`}>
                    <button
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center border ${
                          isOpen ? "bg-[var(--accent-blue)] border-[var(--accent-blue)] text-white" : "bg-white border-[var(--border-hairline)] text-[var(--ink)]"
                        }`}>
                          <span className="text-sm font-bold">{category.label.charAt(0)}</span>
                        </div>
                        <div>
                          <p className={`font-bold text-lg ${isOpen ? "text-[var(--ink)]" : "text-[var(--ink)]"}`}>{category.label}</p>
                          <p className="text-xs text-[var(--ink-soft)] font-medium">{questions[category.id].length} assessment prompts</p>
                        </div>
                      </div>
                      <span className={`p-2 rounded-full transition-all ${isOpen ? "bg-[var(--bg-wash-start)] text-[var(--accent-blue)] rotate-180" : "bg-transparent text-[var(--accent-blue)]"}`}>
                    ...
                    {isOpen && (
                      <div className="px-6 pb-6 pt-2">
                        <div className="border border-[var(--border-hairline)] bg-white p-6 mb-4">
                          <p className="font-semibold text-[var(--ink)] mb-3 flex items-start gap-2">
                            <Compass className="w-5 h-5 text-[var(--accent-blue)] shrink-0" />
                            {insight.headline}
                          </p>
                          <ul className="space-y-3 text-sm text-[var(--ink-soft)] pl-7">
                            {insight.guidance.map((point) => (
                              <li key={point} className="list-disc marker:text-[var(--accent-blue)]">
                                {point}
                              </li>
                            ))}
                          </ul>
