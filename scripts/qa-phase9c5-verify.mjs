...
async function runViewport({ width, height, label, reducedMotion }, baseUrl, outDir) {
  ...
  try {
    const email = `qa.9c5.${label}.${Date.now()}@example.com`;
    ...
    await page.screenshot({ path: path.join(outDir, `01-dashboard-full-${label}.png`), fullPage: true });

    console.log(`[${label}] hero + CTAs`);
    await page.evaluate(() => window.scrollTo(0, 0));
    await page.waitForTimeout(200);
    await page.screenshot({ path: path.join(outDir, `02-hero-${label}.png`) });

    const primaryCta = page.getByRole("button", { name: /Continue Today/i });
    const secondaryCta = page.getByRole("link", { name: /View Full Program/i });
    const primaryVisible = await primaryCta.isVisible().catch(() => false);
    const secondaryVisible = await secondaryCta.isVisible().catch(() => false);
    console.log(`[${label}] primary CTA visible: ${primaryVisible}, secondary CTA visible: ${secondaryVisible}`);

    // Recovery Journey heading + AI Guidance single-card check.
    const journeyHeading = page.getByText("Recovery Journey", { exact: false });
    if (await journeyHeading.isVisible().catch(() => false)) {
      await journeyHeading.scrollIntoViewIfNeeded();
      await page.waitForTimeout(300);
      await page.screenshot({ path: path.join(outDir, `03-recovery-journey-heading-${label}.png`) });
    }

    const aiCoachHeading = page.getByText("What we're noticing", { exact: false });
    if (await aiCoachHeading.isVisible().catch(() => false)) {
      await aiCoachHeading.scrollIntoViewIfNeeded();
      await page.waitForTimeout(300);
      await page.screenshot({ path: path.join(outDir, `04-ai-guidance-${label}.png`) });
    }

    // Optional Modules — collapsed <details> disclosure.
    const optionalSummary = page.getByText("Account & Rewards", { exact: false });
    if (await optionalSummary.isVisible().catch(() => false)) {
      await optionalSummary.scrollIntoViewIfNeeded();
      await page.waitForTimeout(200);
      await page.screenshot({ path: path.join(outDir, `05-optional-collapsed-${label}.png`) });
      await optionalSummary.click();
      await page.waitForTimeout(400);
      await page.screenshot({ path: path.join(outDir, `06-optional-expanded-${label}.png`) });
    }
...
async function main() {
  ...
  const outDir = path.join(workspace, "playwright-artifacts", "design-evolution", "9C", "9C.5");
