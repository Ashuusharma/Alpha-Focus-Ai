...
export default function HomeDashboard({
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-[#F4EFE6] via-[#EFE8DD] to-[#E5E0D4] px-4 py-6 text-[#1F3D2B] sm:px-6 sm:py-8 lg:px-8">
      <section className="mx-auto max-w-6xl space-y-6">
        ...
        <section className="rounded-xl border border-white/60 bg-white/60 p-6 shadow-sm backdrop-blur-md flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
          <div>
             <h1 className="text-2xl font-bold text-[#1F3D2B]">{greeting}</h1>
             <div className="flex items-center gap-4 mt-2 text-sm text-[#6B665D]">
               <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#2F6F57]"></span> Streak: {streakDays} days</span>
               <span>•</span>
               <span>Last updated: {lastUpdated.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
             </div>
          </div>
          <div className="flex gap-3">
             <Link href="/image-analyzer" className="px-5 py-2.5 bg-[#2F6F57] text-white rounded-xl font-semibold text-sm shadow-lg shadow-[#2F6F57]/20 hover:bg-[#1F4D3B] transition-colors">
               New Scan
             </Link>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-white/60 bg-white/60 p-6 shadow-sm backdrop-blur-md">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-lg font-bold text-[#1F3D2B]">Performance Trend</h2>
            <div className="flex bg-white/40 p-1 rounded-lg border border-white/40">
              <button
                onClick={() => setTrendRange("7d")}
                className={`px-3 py-1 rounded-md text-xs font-semibold transition-all ${trendRange === "7d" ? "bg-[#2F6F57] text-white shadow-sm" : "text-[#6B665D] hover:text-[#1F3D2B]"}`}
              >
                7 Days
              </button>
              <button
                onClick={() => setTrendRange("30d")}
                className={`px-3 py-1 rounded-md text-xs font-semibold transition-all ${trendRange === "30d" ? "bg-[#2F6F57] text-white shadow-sm" : "text-[#6B665D] hover:text-[#1F3D2B]"}`}
              >
                30 Days
              </button>
            </div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={visibleTrendData}>
                ...
                <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fill: "#6B665D", fontSize: 10 }} dy={10} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: "#6B665D", fontSize: 10 }} domain={[0, 100]} />
                <Tooltip 
                  contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
                  itemStyle={{ fontSize: '12px', fontWeight: 600 }}
                />
                <Line type="monotone" dataKey="alpha" stroke="#2F6F57" strokeWidth={3} dot={false} activeDot={{ r: 6, fill: "#2F6F57" }} />
        ...
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <section className="rounded-2xl border border-white/60 bg-white/60 p-6 shadow-sm backdrop-blur-md h-full">
            <h3 className="text-lg font-bold text-[#1F3D2B] mb-4">Daily Protocol</h3>
          ...
          <section className="rounded-2xl border border-white/60 bg-white/60 p-6 shadow-sm backdrop-blur-md h-full flex flex-col">
            <h3 className="text-lg font-bold text-[#1F3D2B] mb-4">Clinical Insight</h3>
            <div className="flex-1 bg-white/40 rounded-xl p-5 border border-white/40">
              <p className="text-[#1F3D2B] italic font-medium leading-relaxed">"{aiInsight}"</p>
            </div>
            <div className="mt-4 grid grid-cols-2 gap-4">
               <div className="bg-[#EFE8DD] p-3 rounded-xl">
                 <p className="text-xs text-[#6B665D] uppercase tracking-wider mb-1">Status</p>
                 <p className="text-sm font-bold text-[#1F3D2B]">{activityStatus.completedToday ? "Optimized" : "Pending"}</p>
               </div>
               <div className="bg-[#EFE8DD] p-3 rounded-xl">
                 <p className="text-xs text-[#6B665D] uppercase tracking-wider mb-1">Signals</p>
                 <p className="text-sm font-bold text-[#1F3D2B]">{recentActivityCount} Inputs</p>
               </div>
            </div>
          </section>
        </section>
        ...
        <section className="rounded-2xl border border-white/60 bg-white/60 p-6 shadow-sm backdrop-blur-md">
          <div className="flex items-center justify-between mb-4">
             <h3 className="text-lg font-bold text-[#1F3D2B]">Environmental Impact</h3>
             ...
             {[
               ...
               { label: "Humidity", value: environmentMetrics.humidity ?? "--", unit: "%", desc: "Hydration" },
                ...
                <div key={metric.label} className="bg-white/40 rounded-xl p-4 border border-white/40">
                  <p className="text-xs text-[#6B665D] uppercase tracking-wider">{metric.desc}</p>
                  <p className="text-2xl font-bold text-[#1F3D2B] mt-1">{metric.value}<span className="text-sm text-[#6B665D] ml-1">{metric.unit}</span></p>
                </div>
             ))}
          </div>
          <p className="text-sm text-[#6B665D] bg-[#F4EFE6] p-3 rounded-xl border border-[#E2DDD4]">
            <span className="font-bold text-[#2F6F57] mr-1">Recommendation:</span> {environmentNote}
          </p>
        </section>

        {/* SECTION 6: Next Action */}
        <section className="bg-[#2F6F57] rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl relative overflow-hidden">
           ...
           <div className="relative z-10 text-center md:text-left">
              <p className="text-[#A9CBB7] text-sm font-bold uppercase tracking-widest mb-2">Immediate Protocol</p>
