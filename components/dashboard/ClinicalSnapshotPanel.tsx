  ...
  return (
    <div className="grid h-full grid-rows-[auto_auto_1fr_auto] gap-4 rounded-2xl border border-[#CFC6B8] bg-[linear-gradient(180deg,#F8F5EF_0%,#F2EEE6_100%)] p-4 sm:p-5 shadow-[0_14px_32px_rgba(0,0,0,0.08)]">
      <section className="rounded-xl border border-[#DDD6CA] bg-[#F7F4EE] p-4">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-[#0071e3]">
        ...
        <p className="mt-2 text-xs text-[#0071e3]">Primary Concern: <span className="font-semibold text-[#1E4D3A]">{primaryConcern}</span></p>
      </section>
        ...
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="inline-flex rounded-full border border-[#D9D2C7] bg-white p-0.5 text-xs">
            <button
              type="button"
              onClick={() => setRange("7d")}
              className={`rounded-full px-2.5 py-1 ${range === "7d" ? "bg-[#eef5ff] text-[#1E4D3A]" : "text-[#6E9F87]"}`}
            >
              7-day
            </button>
            <button
              ...
              className={`rounded-full px-2.5 py-1 ${range === "30d" ? "bg-[#eef5ff] text-[#1E4D3A]" : "text-[#6E9F87]"}`}
            >
              30-day
            </button>
          </div>
        ...
        <div className="h-48 sm:h-52 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={safeTrendData} margin={{ top: 4, right: 8, left: -22, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="alpha" stroke="#0071e3" strokeWidth={2.2} dot={false} name="Alpha Score" />
      ...
      <section className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        <div className="rounded-xl bg-[#eef5ff] p-3">
          <p className="text-[11px] uppercase tracking-wide text-[#0071e3]">Next Recommended Action</p>
